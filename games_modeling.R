...
getFeatureInfluence <- function(games.stats, team, season) {
  ...
  return (model_features_var)
}

mostInfluentFeatures <- function(dataframe, features_num) {
  most_influent_features <- head(dataframe[order(-dataframe$variance_diff), , drop = FALSE],features_num)$features
